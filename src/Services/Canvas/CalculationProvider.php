...
class CalculationProvider
{
        ...
        switch (get_class($entity)) {
            ...
            case Paiement::class:
                ...
                if ($offre = $entity->getOffreIndemnisationSinistre()) {
                    ...
                    // Essayer de remonter à la police via les articles de la note
            ...
            case Client::class:
                /** @var Client $entity */
                // On récupère les stats globales filtrées pour ce client
                $stats = $this->getIndicateursGlobaux($entity->getEntreprise(), false, ['clientCible' => $entity]);
            ...
            case Assureur::class:
                ...
                $stats = $this->getIndicateursGlobaux($entity->getEntreprise(), false, ['assureurCible' => $entity]);
            ...
            case Partenaire::class:
                ...
                // On récupère les stats globales filtrées pour ce partenaire
            ...
            case Risque::class:
                /** @var Risque $entity */
                // On récupère les stats globales filtrées pour ce risque
                $stats = $this->getIndicateursGlobaux($entity->getEntreprise(), false, ['risqueCible' => $entity]);
            ...
            case Groupe::class:
                ...
                $stats = $this->getIndicateursGlobaux($entity->getEntreprise(), false, ['groupeCible' => $entity]);
                ...
                $indicateurs = [
                    ...
                    // Mapping des stats globales vers les attributs de l'entité (Agrégat)
                    'primeTotale' => round($stats['prime_totale'], 2),
                    'primePayee' => round($stats['prime_totale_payee'], 2),
                    'primeSoldeDue' => round($stats['prime_totale_solde'], 2),
                    'tauxCommission' => round($stats['taux_de_commission'], 2),
    ...
    private function getInterpretationTauxSP(float $taux): string
    {
        if ($taux == 0) {
