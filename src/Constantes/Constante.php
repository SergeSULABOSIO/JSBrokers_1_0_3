...
class Constante
{
    ...
    public function Entreprise_getSynthseCollecteRevenus()
    {
        ...
        $syntheseRevenu = [];
        $tabTypeRevenus = $this->Entreprise_getTypeRevenus();
    ...
    public function Entreprise_getSynthesePrimes()
    {
        ...
        foreach ($this->Entreprise_getChargements() as $typeChargement) {
            $montant = 0;
    ...
    public function Entreprise_getDataProductionPerRisk()
    {
        ...
        /** @var Avenant $avenant */
        foreach ($this->Entreprise_getRisques() as $risque) {
    ...
    public function Notification_Sinistre_getStatusDocumentsAttendus(?NotificationSinistre $notification_sinistre)
    {
        ...
        if ($notification_sinistre != null) {
            $strgDocs = "";
            $modelesPieces = $this->Entreprise_getModelePieceSinistres();
            foreach ($modelesPieces as $modele) {
                $strgDocs .= $modele->getNom() . ", ";
            }
            $tabDocuments['Attendus'] = $modelesPieces;
            ...
            foreach ($modelesPieces as $typePiece) {
                $isFournis = false;
                foreach ($notification_sinistre->getPieces() as $pieceSinistre) {
    ...
    /**
     * Les entités liées (assureurs, clients, risques…) ne sont plus des collections
     * directes de l'Entreprise : on les dérive du graphe Invites → Pistes → Cotations,
     * exactement comme le fait DashboardDataProvider.
     */
        ...
        foreach ($this->getEnterprise()->getInvites() as $invite) {
            foreach ($invite->getPistes() as $piste) {
                foreach ($piste->getCotations() as $cotation) {
                    ...
                    if ($assureur != null && !$assureurs->contains($assureur)) {
        ...
        foreach ($this->getEnterprise()->getInvites() as $invite) {
            foreach ($invite->getPistes() as $piste) {
                foreach ($piste->getCotations() as $cotation) {
                    ...
                    if ($partenaire != null && !$partenaires->contains($partenaire)) {
    ...
    public function Entreprise_getRisques()
    {
        $risques = new ArrayCollection();
        /** @var Invite $invite */
        foreach ($this->getEnterprise()->getInvites() as $invite) {
            foreach ($invite->getPistes() as $piste) {
                $risque = $piste->getRisque();
                if ($risque != null && !$risques->contains($risque)) {
                    $risques->add($risque);
                }
            }
        }
    ...
    public function Entreprise_getTypeRevenus()
    {
        $typeRevenus = new ArrayCollection();
        /** @var Invite $invite */
        foreach ($this->getEnterprise()->getInvites() as $invite) {
            foreach ($invite->getPistes() as $piste) {
                foreach ($piste->getCotations() as $cotation) {
                    foreach ($cotation->getRevenus() as $revenu) {
                        ...
                        if ($typeRevenu != null && !$typeRevenus->contains($typeRevenu)) {
                            $typeRevenus->add($typeRevenu);
                        }
                    }
                }
            }
    ...
    /**
     * Types de chargement distincts (objets Chargement) effectivement utilisés
     * dans les cotations de l'entreprise.
     */
    public function Entreprise_getChargements()
    {
        ...
        foreach ($this->getEnterprise()->getInvites() as $invite) {
            foreach ($invite->getPistes() as $piste) {
                foreach ($piste->getCotations() as $cotation) {
                    foreach ($cotation->getChargements() as $chargementPourPrime) {
                        $type = $chargementPourPrime->getType();
    ...
    public function Entreprise_getModelePieceSinistres()
    {
        return new ArrayCollection(
            $this->modelePieceSinistreRepository->findBy(['entreprise' => $this->getEnterprise()])
        );
    }
    ...
    public function createClaimReportSet($number, NotificationSinistre $notification): ClaimReportSet
    {
        ...
        return (new ClaimReportSet())
            ...
            ->setVictim(mb_substr((string) $notification->getDescriptionVictimes(), 0, 15) . "[..]")
            ->setCirconstance(mb_substr((string) $notification->getDescriptionDeFait(), 0, 15) . "[..]")
